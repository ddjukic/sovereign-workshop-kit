...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --lightning)  LIGHTNING=true; shift ;;
        -h|--help)
            echo "Usage: ./setup.sh --openrouter <key> [--langdock <key>] [--lightning] [--dry-run]"
            ...
            echo "  --lightning          Lightning.ai mode (auto-reads \$LIGHTNING_API_KEY from env)"
...
# ─── Lightning.ai: resolve API key from environment ─────────────────────────
LIGHTNING_KEY=""
if [[ "$LIGHTNING" == true ]]; then
    if [[ -z "${LIGHTNING_API_KEY:-}" ]]; then
        fail "--lightning requires \$LIGHTNING_API_KEY in your environment."
        echo ""
        echo "  This variable is set automatically in Lightning.ai Studios."
        echo "  If you're not in a Lightning Studio, set it manually:"
...
if [[ "$DRY_RUN" == true ]]; then
    ...
    if [[ "$LIGHTNING" == true ]]; then
        ok "Lightning: enabled (gateway bind=lan, trustedProxies=any)"
    else
        info "Lightning: disabled (gateway bind=loopback)"
    fi
...
[[ "$LIGHTNING" == true ]] && echo "  Lightning mode: enabled (key from \$LIGHTNING_API_KEY)"
echo ""
...
if [[ "$LIGHTNING" == true ]]; then
    ...
    echo -e "  ${BOLD}https://18789-${LIGHTNING_CLOUD_SPACE_ID:-\$CLOUD_SPACE_ID}.cloudspaces.litng.ai/?token=${LIGHTNING_KEY}${NC}"
    echo ""
    echo -e "  ${DIM}If the dashboard shows 'pairing required', run in your terminal:${NC}"
    echo -e "  ${DIM}  openclaw devices list${NC}"
    echo -e "  ${DIM}  openclaw devices approve <request-id>${NC}"
